...
function BadgeList({ className, badges }: BadgeListProps) {
  ...
  return (
    <ul
      className={cn("flex list-none flex-wrap gap-x-1 p-0", className)}
    ...
    >
      {badges.map((badge) => (
        <li key={badge}>
          <Badge
            variant="secondary"
            className="mb-1 align-middle text-xs print:px-1 print:py-0.5 print:text-[8px] print:leading-tight"
          >
            {badge}
          </Badge>
        </li>
